...
def main_update_indexer(max_items_to_process=50):
    initialize_models()
    ...
    # Identify already indexed items by 'english_pdf_link' OR filename
    ...
    logger.info(f"Found {len(unindexed_items)} unindexed items.")
    ...
    def process_single_item(item_data):
        ...
        try:
            ...
            return convert_item_blocks_to_texts_and_metadata(merged_blocks, item_data)
    ...
    for result in results:
        if result:
            texts, metadata_for_item = result
