...
def _process_page(pdf_path, page_num, table_detector_pipeline, ocr_reader):
...
def extract_content_from_pdf(pdf_path, table_detector_pipeline, ocr_reader, max_workers=4):
    """
    Extracts structured content (text and tables) from a single PDF file using parallel processing.
    ...
    Args:
        ...
        max_workers (int): Maximum number of threads for parallel page processing.
    ...
    try:
        ...
        num_pages = len(doc)
        doc.close() # Close immediately since convert_from_path opens it again
    except Exception as e:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(_process_page, pdf_path, page_num, table_detector_pipeline, ocr_reader): page_num 
    ...
    extracted_pdf_data = sorted(results, key=lambda x: x['page_number'])
    return extracted_pdf_data
