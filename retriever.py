...
def retrieve_relevant_chunks(query_text,
                             ...
                             top_n_initial=getattr(config, 'TOP_N_INITIAL_RETRIEVAL', 20),
                             top_n_final=config.TOP_N_RETRIEVAL):
    """
    Retrieves the top_n most relevant text chunks from the FAISS index for a given query,
    optionally re-ranking them with a CrossEncoder.
    ...
    Args:
        ...
        cross_encoder_model: Initialized CrossEncoder model for re-ranking (optional).
        top_n_initial (int): Number of chunks to retrieve initially before re-ranking.
        top_n_final (int): Number of chunks to return after re-ranking.
    ...
    try:
        ...
        logger.info(f"Searching FAISS index for top {top_n_initial} chunks...")
        # D are distances (L2 distance for IndexFlatL2), I are the indices/IDs of the vectors
        distances, indices = faiss_index.search(query_embedding_np, k=top_n_initial)
        ...
        if cross_encoder_model and retrieved_results:
            logger.info("Re-ranking retrieved chunks using CrossEncoder...")
            cross_input = [[query_text, result["text"]] for result in retrieved_results]
            cross_scores = cross_encoder_model.predict(cross_input)
            
            for i, score in enumerate(cross_scores):
                retrieved_results[i]["score"] = float(score) # Update with cross-encoder score
                
        # Sort by score descending (higher similarity is better)
        retrieved_results.sort(key=lambda x: x['score'], reverse=True)
        
        # Keep only top_n_final
        retrieved_results = retrieved_results[:top_n_final]
